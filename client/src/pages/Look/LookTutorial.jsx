...
export default function LookTutorial() {
  ...
  useEffect(() => {
    const getPost = async () => {
      try {
        const res = await axios.get(`https://${baseUrl}/posts/get/lookTutorial`)
        setPost(res.data)
      } catch (error) {
        console.error(error)
      }
            ...
            <TableBody>
              {post.map((row) => (
                ...
                >
                  ...
                  <TableCell
                    onClick={() => {
                      navigate(`/look/tutorial/${row._id}`)
                    }}
                    sx={{
                      cursor: 'pointer',
                      ':hover': {
